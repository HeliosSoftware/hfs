...
mod tests {
    ...
    fn test_repeat_function_basic() {
        ...
        let object1 = EvaluationResult::Object({
            ...
            map.insert("name".to_string(), EvaluationResult::String("root".to_string()));
            map.insert("child".to_string(), EvaluationResult::Object({
                let mut child_map = std::collections::HashMap::new();
                child_map.insert("name".to_string(), EvaluationResult::String("level1".to_string()));
                child_map.insert("child".to_string(), EvaluationResult::Object({
                    let mut grandchild_map = std::collections::HashMap::new();
                    grandchild_map.insert("name".to_string(), EvaluationResult::String("level2".to_string()));
                    grandchild_map
                }));
                child_map
            }));
        ...
        if let EvaluationResult::Collection(items) = result {
            ...
            for item in items {
                if let EvaluationResult::Object(map) = &item {
    ...
    fn test_repeat_function_circular() {
        ...
        obj1_map.insert("name".to_string(), EvaluationResult::String("obj1".to_string()));
        obj2_map.insert("name".to_string(), EvaluationResult::String("obj2".to_string()));
        
        // Now set circular references 
        // (we need to create the objects first since we can't create circular references directly)
        let obj1_temp = EvaluationResult::Object(obj1_map.clone());
        let obj2_temp = EvaluationResult::Object(obj2_map.clone());
        ...
        let obj1 = EvaluationResult::Object(obj1_map);
        let _obj2 = EvaluationResult::Object(obj2_map); // Prefix with underscore since we don't use it directly
        
        // Now create a root object that refers to these
        let mut root_map = std::collections::HashMap::new();
        root_map.insert("name".to_string(), EvaluationResult::String("root".to_string()));
        root_map.insert("next".to_string(), obj1.clone());
        
        let root = EvaluationResult::Object(root_map);
