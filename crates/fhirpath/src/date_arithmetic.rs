...
pub fn add_date_time_quantity(
    ...
    let (value, unit) = match right {
        EvaluationResult::Quantity(val, unit, _) => (*val, unit.clone()),
        EvaluationResult::Integer(i, _) => (Decimal::from(*i), "day".to_string()), // Assume days if no unit provided
        _ => {
    ...
    match left {
        EvaluationResult::Date(date_str, _) => {
            if let Some(date) = datetime_impl::parse_date(date_str) {
                ...
                match result {
                    Some(new_date) => Ok(EvaluationResult::date(
                        new_date.format("%Y-%m-%d").to_string(),
                    )),
                    None => Err(EvaluationError::TypeError(
            ...
            if let Some(dt) = datetime_impl::parse_datetime(dt_str) {
                let result = match unit.as_str() {
                    ...
                    "millisecond" | "milliseconds" => {
                        dt.checked_add_signed(Duration::milliseconds(amount))
                    }
                    _ => {
                        return Err(EvaluationError::TypeError(format!(
        ...
        EvaluationResult::Time(time_str, _) => {
            if let Some(time) = datetime_impl::parse_time(time_str) {
                let result = match unit.as_str() {
                    ...
                    "minute" | "minutes" => {
                        time.overflowing_add_signed(Duration::minutes(amount)).0
                    }
                    "second" | "seconds" => {
                        time.overflowing_add_signed(Duration::seconds(amount)).0
                    }
                    "millisecond" | "milliseconds" => {
                        time.overflowing_add_signed(Duration::milliseconds(amount))
                            .0
                    }
                    _ => {
                        return Err(EvaluationError::TypeError(format!(
...
pub fn date_time_difference(
    ...
    match (left, right) {
        (EvaluationResult::Date(date1, _), EvaluationResult::Date(date2, _)) => {
            ...
            let diff = match unit {
                ...
                _ => {
                    return Err(EvaluationError::TypeError(format!(
                        "Cannot calculate date difference in {}",
                        unit
                    )));
                }
            };
        ...
        (EvaluationResult::DateTime(dt1, _), EvaluationResult::DateTime(dt2, _)) => {
            ...
            let diff = match unit {
                ...
                _ => {
                    return Err(EvaluationError::TypeError(format!(
                        "Cannot calculate datetime difference in {}",
                        unit
                    )));
                }
            };
        ...
        (EvaluationResult::Time(time1, _), EvaluationResult::Time(time2, _)) => {
            ...
            let diff = match unit {
                ...
                _ => {
                    return Err(EvaluationError::TypeError(format!(
                        "Cannot calculate time difference in {}",
                        unit
                    )));
                }
            };
...
mod tests {
    ...
    fn test_add_time_to_datetime() {
        ...
        assert_eq!(
            result,
            EvaluationResult::datetime("2023-05-15T15:00:45.000Z".to_string())
        );
    }
    ...
    fn test_date_difference_in_days() {
        ...
        assert_eq!(
            result,
            EvaluationResult::quantity(dec!(5), "day".to_string())
        );
    }
    ...
    fn test_datetime_difference_in_hours() {
        ...
        assert_eq!(
            result,
            EvaluationResult::quantity(dec!(2), "hour".to_string())
        );
    }
