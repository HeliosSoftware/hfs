...
pub fn intersect_function(
    ...
    for left_item in &left_items {
        ...
        let exists_in_right = right_items.iter().any(|right_item| {
            equal_helper(left_item, right_item, context)
        });
...
pub fn union_function(
    ...
    for item in right_items {
        if !union_items.iter().any(|existing| equal_helper(&existing, &item, context)) {
            union_items.push(item);
        }
    }
...
fn equal_helper(
    ...
    if let (EvaluationResult::Collection { items: l_items, .. }, EvaluationResult::Collection { items: r_items, .. }) = (left, right) {
        if l_items.len() != r_items.len() {
            return false;
        }
        return l_items.iter().zip(r_items.iter()).all(|(li, ri)| equal_helper(li, ri, _context));
    }
    ...
    match (left, right) {
        ...
        (EvaluationResult::Decimal(l), EvaluationResult::Integer(r)) => *l == rust_decimal::Decimal::from(*r),
        (EvaluationResult::Integer(l), EvaluationResult::Decimal(r)) => rust_decimal::Decimal::from(*l) == *r,
        ...
        (EvaluationResult::Quantity(l_val, l_unit), EvaluationResult::Quantity(r_val, r_unit)) => {
...
mod tests {
    ...
    fn test_combine_with_duplicates() {
        ...
        match result {
            EvaluationResult::Collection { items, has_undefined_order } => {
                ...
                for item in items {
                    match item {
                        EvaluationResult::Integer(1) => count_1 += 1,
                        EvaluationResult::Integer(2) => count_2 += 1,
                        EvaluationResult::Integer(3) => count_3 += 1,
    ...
    fn test_equal_helper() {
        ...
        // Test basic equality
        assert!(equal_helper(&EvaluationResult::Integer(1), &EvaluationResult::Integer(1), &context));
        assert!(!equal_helper(&EvaluationResult::Integer(1), &EvaluationResult::Integer(2), &context));
        
        // Test different types
        assert!(!equal_helper(&EvaluationResult::Integer(1), &EvaluationResult::String("1".to_string()), &context));
        
        // Test Number equality (Integer and Decimal)
        assert!(equal_helper(&EvaluationResult::Integer(1), &EvaluationResult::Decimal(Decimal::from(1)), &context));
        assert!(equal_helper(&EvaluationResult::Decimal(Decimal::from(1)), &EvaluationResult::Integer(1), &context));
        ...
        assert!(equal_helper(&singleton, &EvaluationResult::Integer(1), &context));
        assert!(equal_helper(&EvaluationResult::Integer(1), &singleton, &context));
    }
}