...
pub fn parser() -> impl Parser<char, Expression, Error = Simple<char>> + Clone {
    ...
    let expr_parser = recursive(|expr| {
        ...
        let atom = term.clone().repeated().at_least(1).foldr(|term, expr_opt| {
            match expr_opt {
                Some(expr) => Expression::Invocation(
                    Box::new(Expression::Term(term)), 
                    Invocation::Member(String::new())
                ),
                None => Expression::Term(term)
            }
        }).padded();
        ...
        let indexer_expr = invocation_expr
            ...
            .foldl(|expr, idx| Expression::Indexer(Box::new(expr), Box::new(idx)))
        ...
        let type_expr = additive_expr
            ...
            .map(|(expr, type_op)| {
                if let Some((op, type_spec)) = type_op {
                    Expression::Type(Box::new(expr), op.to_string(), type_spec)
                } else {
                    expr
                }
            })
