...
mod tests {
    ...
    use helios_fhirpath::type_inference::{TypeContext, InferredType};
    use chumsky::Parser;
    ...
    fn test_parse_debug_tree_with_types() {
        ...
        let type_context = TypeContext::new()
            .with_root_type(InferredType::fhir("HumanName"));
        
        // Generate the debug tree
        let debug_tree = expression_to_debug_tree(&parsed, &type_context);
        
        // Pretty print the JSON
        let json_string = serde_json::to_string_pretty(&debug_tree)
        ...
        assert_eq!(debug_tree.get("ExpressionType").and_then(|v| v.as_str()), Some("FunctionCallExpression"));
        assert_eq!(debug_tree.get("Name").and_then(|v| v.as_str()), Some("join"));
    }
