...
fn debug_datetime_boundary_issue() {
    ...
    println!("1. Raw valueDateTime: {:?}", evaluate_expression("valueDateTime", &context).unwrap());
    
    // Check polymorphic value access
    println!("2. Polymorphic value: {:?}", evaluate_expression("value", &context).unwrap());
    
    // Check what valueDateTime field contains
    println!("2a. Checking valueDateTime field: {:?}", evaluate_expression("valueDateTime", &context).unwrap());
    
    // Check basic fields on the observation
    println!("2b. Status field: {:?}", evaluate_expression("status", &context).unwrap());
    
    // Check the id field to make sure basic access works
    println!("2c. ID field: {:?}", evaluate_expression("id", &context).unwrap());
    
    // Check the type casting
    println!("3. value.ofType(dateTime): {:?}", evaluate_expression("value.ofType(dateTime)", &context).unwrap());
    
    // Check what happens with a literal datetime
    println!("4. Literal datetime boundary: {:?}", evaluate_expression("@2010-10-10.lowBoundary()", &context).unwrap());
    
    // Check if the issue is with ofType or lowBoundary
    println!("5. valueDateTime.lowBoundary(): {:?}", evaluate_expression("valueDateTime.lowBoundary()", &context).unwrap());
    
    // Try direct datetime boundary
    println!("6. @2010-10-10T00:00:00.lowBoundary(): {:?}", evaluate_expression("@2010-10-10T00:00:00.lowBoundary()", &context).unwrap());
    
    // Final test - the exact failing expression
    println!("7. value.ofType(dateTime).lowBoundary(): {:?}", 
...
fn debug_datetime_type_conversion() {
    ...
    for date_format in date_formats {
        println!("\nTesting: {}", date_format);
        println!("  lowBoundary: {:?}", evaluate_expression(&format!("{}.lowBoundary()", date_format), &context).unwrap());
        println!("  highBoundary: {:?}", evaluate_expression(&format!("{}.highBoundary()", date_format), &context).unwrap());
    }
}