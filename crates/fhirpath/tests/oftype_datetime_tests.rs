...
fn test_oftype_datetime_conversion() {
    ...
    println!("Date: {:?}", evaluate_expression("@2010-10-10", &context).unwrap());
    println!("Date.ofType(dateTime): {:?}", evaluate_expression("@2010-10-10.ofType(dateTime)", &context).unwrap());
    println!("Date.ofType(System.DateTime): {:?}", evaluate_expression("@2010-10-10.ofType(System.DateTime)", &context).unwrap());
    
    // Test DateTime boundaries after ofType conversion
    println!("Date.ofType(dateTime).lowBoundary(): {:?}", 
