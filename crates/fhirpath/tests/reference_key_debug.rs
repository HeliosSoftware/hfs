...
fn debug_reference_key_functions() {
    ...
    for (name, patient) in [("p1", patient_p1), ("p2", patient_p2)] {
        ...
        println!("getResourceKey(): {:?}", evaluate_expression("getResourceKey()", &context).unwrap());
        println!("link.other: {:?}", evaluate_expression("link.other", &context).unwrap());
        println!("link.other.reference: {:?}", evaluate_expression("link.other.reference", &context).unwrap());
        println!("link.other.getReferenceKey(): {:?}", evaluate_expression("link.other.getReferenceKey()", &context).unwrap());
        println!("link.other.getReferenceKey(Patient): {:?}", evaluate_expression("link.other.getReferenceKey(Patient)", &context).unwrap());
        println!("link.other.getReferenceKey('Patient'): {:?}", evaluate_expression("link.other.getReferenceKey('Patient')", &context).unwrap());
        
        // Test what Patient evaluates to
        println!("Patient identifier: {:?}", evaluate_expression("Patient", &context).unwrap());
        
        // Test the full expressions
        println!("Without type filter: {:?}", evaluate_expression("getResourceKey() = link.other.getReferenceKey()", &context).unwrap());
        println!("With type filter: {:?}", evaluate_expression("getResourceKey() = link.other.getReferenceKey(Patient)", &context).unwrap());
