...
mod tests {
    ...
    use serde_json::json;
    use axum::{Json, body, response::Response};
    ...
    async fn test_trace_output_in_response() {
        ...
        let params: FhirPathParameters = serde_json::from_value(params_json)
        ...
        // Find trace parts
        let trace_parts: Vec<_> = parts.iter()
            .filter(|p| p["name"] == "trace")
            .collect();
    ...
    async fn test_trace_with_projection() {
        ...
        println!("Response with projection: {}", serde_json::to_string_pretty(&body).unwrap());
        ...
        let values: Vec<i64> = trace_values.iter()
