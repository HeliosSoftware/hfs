...
fn bench_simple_expressions(c: &mut Criterion) {
    ...
    group.bench_function("single_path", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name"))
        })
    });
    
    group.bench_function("nested_path", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name.family"))
        })
    });
    
    group.bench_function("indexed_access", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name[0].given[1]"))
        })
    });
    
    group.bench_function("boolean_literal", |b| {
        b.iter(|| {
            parser().parse(black_box("true"))
        })
    });
    
    group.bench_function("integer_literal", |b| {
        b.iter(|| {
            parser().parse(black_box("42"))
        })
    });
    
    group.bench_function("string_literal", |b| {
        b.iter(|| {
            parser().parse(black_box("'hello world'"))
        })
    });
...
fn bench_function_calls(c: &mut Criterion) {
    ...
    group.bench_function("single_function", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name.first()"))
        })
    });
    
    group.bench_function("function_with_args", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.telecom.where(system = 'email')"))
        })
    });
    
    group.bench_function("chained_functions", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name.given.first().upper()"))
        })
    });
    
    group.bench_function("nested_function_calls", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.name.where(use = 'official').given.select(substring(0, 1))"))
        })
    });
...
fn bench_operators(c: &mut Criterion) {
    ...
    group.bench_function("comparison", |b| {
        b.iter(|| {
            parser().parse(black_box("value > 5"))
        })
    });
    
    group.bench_function("equality", |b| {
        b.iter(|| {
            parser().parse(black_box("status = 'active'"))
        })
    });
    
    group.bench_function("arithmetic", |b| {
        b.iter(|| {
            parser().parse(black_box("value + 10 * 2"))
        })
    });
    
    group.bench_function("boolean_logic", |b| {
        b.iter(|| {
            parser().parse(black_box("active and (status = 'final' or status = 'amended')"))
        })
    });
    
    group.bench_function("union", |b| {
        b.iter(|| {
            parser().parse(black_box("name.given | name.family"))
        })
    });
...
fn bench_complex_expressions(c: &mut Criterion) {
    ...
    group.bench_function("complex_filter", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.telecom.where(system = 'phone' and use = 'mobile').value"))
        })
    });
    
    group.bench_function("type_checking", |b| {
        b.iter(|| {
            parser().parse(black_box("value.ofType(Quantity).where(value > 140 and unit = 'mm[Hg]')"))
        })
    });
    
    group.bench_function("extension_access", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.extension('http://example.org/birthPlace').value.ofType(Address).city"))
        })
    });
    
    group.bench_function("aggregate_expression", |b| {
        b.iter(|| {
            parser().parse(black_box("Bundle.entry.resource.ofType(Observation).value.aggregate($total + $this, 0)"))
        })
    });
    
    group.bench_function("date_arithmetic", |b| {
        b.iter(|| {
            parser().parse(black_box("Patient.birthDate + 1 year <= today()"))
        })
    });
...
fn bench_large_expressions(c: &mut Criterion) {
    ...
    group.bench_function("many_or_conditions", |b| {
        b.iter(|| {
            parser().parse(black_box(many_ors.as_str()))
        })
    });
    ...
    group.bench_function("deep_nesting", |b| {
        b.iter(|| {
            parser().parse(black_box(deep_nesting))
        })
    });
    
    // Expression with many function calls
    let many_functions = "Patient.name.given.first().upper().substring(0, 3).replace('A', 'B').trim()";
    
    group.bench_function("many_functions", |b| {
        b.iter(|| {
            parser().parse(black_box(many_functions))
        })
    });
