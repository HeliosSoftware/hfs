...
fn create_test_bundle(patients: Vec<serde_json::Value>) -> Result<SofBundle, Box<dyn std::error::Error>> {
...
fn create_view_definition(json: serde_json::Value) -> Result<SofViewDefinition, Box<dyn std::error::Error>> {
    #[cfg(feature = "R4")]
    {
...
fn test_basic_view_definition_execution() {
    ...
    // Test CSV output
    let csv_result = run_view_definition(view_definition.clone(), bundle.clone(), ContentType::CsvWithHeader);
...
fn test_csv_format_variations() {
    ...
    let csv_with_headers = run_view_definition(view_definition.clone(), bundle.clone(), ContentType::CsvWithHeader).unwrap();
    ...
    let csv_without_headers = run_view_definition(view_definition, bundle, ContentType::Csv).unwrap();
...
fn test_output_format_consistency() {
    ...
    let json_result = run_view_definition(view_definition.clone(), bundle.clone(), ContentType::Json).unwrap();
    ...
    let csv_result = run_view_definition(view_definition, bundle, ContentType::CsvWithHeader).unwrap();
...
fn test_empty_bundle() {
    ...
    let json_result = run_view_definition(view_definition.clone(), empty_bundle.clone(), ContentType::Json).unwrap();
    let json_str = String::from_utf8(json_result).unwrap();
    let json_data: Vec<serde_json::Value> = serde_json::from_str(&json_str).unwrap();
    assert_eq!(json_data.len(), 0);
    
    let csv_result = run_view_definition(view_definition, empty_bundle, ContentType::CsvWithHeader).unwrap();
    let csv_str = String::from_utf8(csv_result).unwrap();
