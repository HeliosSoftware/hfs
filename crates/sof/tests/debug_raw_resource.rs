...
fn test_raw_fhir_parsing() {
    ...
    // Parse as R4 Patient
    let patient: fhir::r4::Patient = serde_json::from_value(patient_json.clone()).expect("Failed to parse patient");
    ...
    if let Some(extensions) = &patient.extension {
        if let Some(first_ext) = extensions.first() {
            ...
            if let Some(value) = &first_ext.value {
                ...
                println!("Value structure: {}", serde_json::to_string_pretty(&value_json).unwrap());
            } else {
