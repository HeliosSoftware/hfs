...
fn test_shortened_format_names() -> Result<(), SofError> {
    ...
    assert_eq!(ContentType::from_string("text/csv")?, ContentType::CsvWithHeader);
    assert_eq!(ContentType::from_string("text/csv;header=true")?, ContentType::CsvWithHeader);
    assert_eq!(ContentType::from_string("text/csv;header=false")?, ContentType::Csv);
    assert_eq!(ContentType::from_string("application/json")?, ContentType::Json);
    assert_eq!(ContentType::from_string("application/ndjson")?, ContentType::NdJson);
    assert_eq!(ContentType::from_string("application/parquet")?, ContentType::Parquet);
