...
fn test_two_selects_with_columns() {
    ...
    let bundle: fhir::r4::Bundle = serde_json::from_value(bundle_json).expect("Failed to create bundle");
    ...
    let view_definition: fhir::r4::ViewDefinition = serde_json::from_value(view).expect("Failed to create ViewDefinition");
    let sof_view = SofViewDefinition::R4(view_definition);
    
    // Run the view definition
    let result = run_view_definition(sof_view, sof_bundle, ContentType::Json).expect("Failed to run ViewDefinition");
    
    // Parse result
    let actual_rows: Vec<serde_json::Value> = serde_json::from_slice(&result).expect("Failed to parse result");
    ...
    for (i, actual_row) in actual_rows.iter().enumerate() {
        ...
        if let (Some(actual_obj), Some(expected_obj)) = (actual_row.as_object(), expected_row.as_object()) {
            // Check that both id and last_name columns are present
            assert!(actual_obj.contains_key("id"), "Row {} missing 'id' column", i);
            assert!(actual_obj.contains_key("last_name"), "Row {} missing 'last_name' column", i);
            
            // Check that values match
            for (key, expected_val) in expected_obj {
                match actual_obj.get(key) {
                    Some(actual_val) => {
                        assert_eq!(actual_val, expected_val, "Row {} column '{}' value mismatch", i, key);
                    }
                    None => {
