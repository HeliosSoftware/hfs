...
async fn test_format_parameter_in_body_csv() {
    ...
    assert!(lines[0].contains("id") && lines[0].contains("birthDate") && lines[0].contains("family") && lines[0].contains("given"));
    assert!(lines[1].contains("Patient/pt-1") && lines[1].contains("2012-03-30") && lines[1].contains("Cole") && lines[1].contains("Joanie"));
    assert!(lines[2].contains("Patient/pt-2") && lines[2].contains("2012-03-30") && lines[2].contains("Doe") && lines[2].contains("John"));
}
...
async fn test_format_parameter_in_body_overrides_accept_header() {
    ...
    assert_eq!(response.status_code(), StatusCode::OK);
    let content_type = response.header("content-type");
    assert_eq!(content_type.to_str().unwrap(), "text/csv", "_format parameter should override Accept header");
...
async fn test_format_parameter_in_body_overrides_query_parameter() {
    ...
    assert_eq!(response.status_code(), StatusCode::OK);
    let content_type = response.header("content-type");
    assert_eq!(content_type.to_str().unwrap(), "text/csv", "_format parameter in body should override query parameter");
}
