...
fn test_base64_binary_constant_type() {
    ...
    // Load test file
    let test_file_path = "../../tests/sql-on-fhir/sql-on-fhir-v2/tests/constant_types.json";
    ...
    match run_view_definition(view_definition, bundle, ContentType::Json) {
        Ok(result) => {
            ...
            if let Some(expected) = &base64_test.expect {
                assert_eq!(actual_rows.len(), expected.len(), 
                    "Result count mismatch: got {}, expected {}", actual_rows.len(), expected.len());
        ...
        Err(e) => {
            ...
            if e.to_string().contains("Unsupported constant type") {
                panic!("The Canonical constant type fix does not appear to be working - still getting 'Unsupported constant type' error: {}", e);
            } else {
