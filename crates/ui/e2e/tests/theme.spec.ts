...
test("a returning dark-mode user sees no flash of light", async ({ page }) => {
  await page.addInitScript(() => {
    try {
      localStorage.setItem("hfs-theme", "dark");
    } catch {}
  });
...
test("toggling roams the choice via an RFC 7386 merge-patch", async ({ page }) => {
...
test("a roamed server choice wins over the local cache", async ({ page }) => {
  ...
  await page.addInitScript(() => {
    try {
      localStorage.setItem("hfs-theme", "light");
    } catch {}
  });
  await page.goto("/ui", { waitUntil: "networkidle" });
  await expect(page.locator("html")).toHaveAttribute("data-theme", "dark");
...
test("an unavailable settings store degrades to the cached choice", async ({ page }) => {
