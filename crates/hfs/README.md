...
| `HFS_REINDEX_BATCH_SIZE` | `1000` | Page size of the automatic deferred rebuild that follows a bulk import, on every backend. `POST $reindex` keeps its own `batchSize` parameter (default 100), capped at 10,000. |
