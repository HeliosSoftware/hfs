...
mod postgres_integration {
    ...
    /// #1092: `_id` is answered from `resources.id` through a dedicated
    /// builder that used to read only the values, so `_id:not=abc` returned
    /// exactly `abc` — the precise inverse of the request — with a 200. The
    /// SQL shape is pinned by the builder's unit tests; this confirms the page
    /// and the count agree against a live database.
    #[tokio::test]
    async fn postgres_integration_search_id_not_modifier() {
        use helios_persistence::core::SearchProvider;
        use helios_persistence::types::{
            SearchModifier, SearchParamType, SearchParameter, SearchQuery, SearchValue,
        };
        ...
        let backend = create_backend().await;
        let tenant = create_tenant("id-not-modifier");
