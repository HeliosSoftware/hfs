...
/// `%rowIndex` over the native Mongo aggregation runner: `$unwind`'s
/// `includeArrayIndex` must surface the 0-based element position, including the
/// `forEachOrNull` empty case (null index → 0).
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn mongodb_integration_sof_row_index() {
    use futures::StreamExt;
    use helios_persistence::core::ViewFilters;

    let Some(backend) = create_backend("rowindex").await else {
        eprintln!("Skipping mongodb_integration_sof_row_index (set HFS_TEST_MONGODB_URL)");
        return;
    };
    let tenant = create_tenant("tenant-ri");

    // One patient with two names, one with none (exercises forEachOrNull).
    backend
        .create(
            &tenant,
            "Patient",
            json!({"resourceType": "Patient", "name": [{"family": "Smith"}, {"family": "Jones"}]}),
            FhirVersion::default(),
        )
        .await
        .unwrap();
    backend
        .create(
            &tenant,
            "Patient",
            json!({"resourceType": "Patient", "id": "noname"}),
            FhirVersion::default(),
        )
        .await
        .unwrap();
    ...
    let runner = backend
        .sof_runner()
        .expect("MongoBackend must provide a SOF runner");
    ...
    // forEach: index within the name collection (0, 1); the no-name patient
    // contributes no rows.
    let fe = rows(
        &runner,
        &tenant,
        json!({
            ...
            "select": [{"forEach": "name", "column": [
                {"name": "idx", "path": "%rowIndex", "type": "integer"},
                {"name": "family", "path": "family", "type": "string"}]}]
        }),
    )
    .await;
    let mut fe_pairs: Vec<(i64, String)> = fe
        .iter()
        .map(|r| {
            (
                r["idx"].as_i64().unwrap(),
                r["family"].as_str().unwrap().to_string(),
            )
        })
        .collect();
    fe_pairs.sort();
    // names = [Smith, Jones] → Smith at index 0, Jones at index 1.
    assert_eq!(
        fe_pairs,
        vec![(0, "Smith".to_string()), (1, "Jones".to_string())],
        "forEach %rowIndex pairs"
    );

    // forEachOrNull over the no-name patient: one null row with %rowIndex = 0.
    let fon = rows(
        &runner,
        &tenant,
        json!({
            "resourceType": "ViewDefinition", "resource": "Patient", "status": "active",
            "select": [{"forEachOrNull": "name", "column": [
                {"name": "idx", "path": "%rowIndex", "type": "integer"},
                {"name": "family", "path": "family", "type": "string"}]}]
        }),
    )
    .await;
    // The no-name patient yields exactly one row with idx 0 and a null family.
    let null_rows: Vec<_> = fon
        .iter()
        .filter(|r| r.get("family").map(|f| f.is_null()).unwrap_or(true))
        .collect();
    assert_eq!(
        null_rows.len(),
        1,
        "expected one forEachOrNull null row: {fon:?}"
    );
    assert_eq!(null_rows[0]["idx"].as_i64().unwrap(), 0);

    // Top level: %rowIndex is always 0.
    let top = rows(
        &runner,
        &tenant,
        json!({
    ...
    assert!(!top.is_empty());
    assert!(
        top.iter().all(|r| r["idx"].as_i64() == Some(0)),
        "top-level rows: {top:?}"
    );
}
