...
///
/// v30 moves the reachability proof into the operator instead: `~>=~` and `~~`
...
impl PostgresQueryBuilder {
    ...
    ///   served by the trigram GIN index `idx_search_string_trgm` (v30), and by
    ...
    fn build_string_condition(param: &SearchParameter, offset: usize) -> Option<SqlFragment> {
        ...
        for value in param.values.iter() {
            let condition = match modifier {
                ...
                // trigram GIN index v30 adds — which is why the
                ...
                // pattern index serves it at parity with the pre-v30 plan.
