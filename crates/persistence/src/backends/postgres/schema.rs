...
/// v30 -> v31: store `value_reference` in its version-agnostic base form, and
...
/// = only  (v31)       0.479  0.494  0.418 ms/call        -3.1x
/// ```
///
...
///
/// - **v30 data, v31 code** cannot happen. `initialize_schema` runs the chain
///   under an advisory lock before the instance serves traffic, and a failure
///   aborts startup rather than continuing.
/// - **v31 data, v30 code** (a binary rollback) is safe because v30's predicate
///   is a strict superset of v31's: `= OR LIKE '<base>/\_history/%'` still
///   matches a row holding `<base>`.
/// - A v31 database written by a *later* rollback to a v30 writer would store
///   suffixes again, and a v31 reader would stop matching those rows. That is
///   the one lossy direction and it requires downgrading the binary, writing,
///   then upgrading; `set_schema_version` leaves the recorded version at 31, so
///   the `UPDATE` does not run again. It is called out here rather than guarded
///   against, because guarding it means keeping the `LIKE` arm, which is the
///   thing being removed.
///
