...
/// ```text
///                                       buffers    time
/// v29 (btree slice scan)                  1,709   1.03 ms
/// v30 (trigram bitmap scan)                  58   0.41 ms
...
/// ```
///
/// Run 33179839720 measured the v29 shape at 5,138 buffers / 10.5 ms on the real
...
/// three runs, against the rest of the v30 index set:
///
/// ```text
...
/// parameter-slice scan measured at 1,709 buffers — the pre-v30 plan exactly.
...
async fn migrate_v29_to_v30(client: &deadpool_postgres::Client) -> StorageResult<()> {
    ...
    // which its strict `~~` also proves, at parity with the pre-v30 plan
    ...
    for sql in trigram_stmts {
        if let Err(e) = client.execute(sql, &[]).await {
            tracing::warn!(
                "Migration v29->v30: trigram index unavailable, `:contains` string \
