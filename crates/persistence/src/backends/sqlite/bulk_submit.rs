...
impl SubmitWorkerStorage for SqliteBackend {
    ...
    async fn finish_manifest(&self, lease: &ManifestLease) -> Result<(), LeaseError> {
        ...
        //
        // `status = 'processing'` is part of that fence, not decoration:
    ...
    async fn fail_manifest(
        ...
        //
        // And the same `status = 'processing'` fence: an aborted manifest's
        // outcome belongs to the abort, so a late worker verdict must not
        // overwrite it — it gets `LeaseLost` instead (#968).
        let affected =
