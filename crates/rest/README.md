...
| `HFS_BULK_SUBMIT_DEFER_INDEXING` | `false` | Bulk fast-load (#903): ingest without search-index/FTS writes, then rebuild them with an automatic per-type reindex when each manifest finishes. Reads and history stay complete throughout; search sees a manifest's resources once its reindex lands. End to end, `true` measured ~1.3x faster to a fully searchable database, though on a loaded machine — read that as a direction rather than a firm number (see `crates/hfs/tests/bulk_submit/run_defer_indexing_benchmark.sh`). Note that the rebuild is in-process and unrecorded: `$bulk-submit-status` returns `200` as soon as the manifest is terminal, which is *before* the rebuild finishes, so a restart in that window leaves the ingested resources stored but unsearchable until someone runs `$reindex` by hand. |
