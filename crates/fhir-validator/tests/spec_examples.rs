...
//! # What this asserts
//!
//! Every published spec example is, by construction, *supposed* to be valid
//! against the core spec. So this is a **false-positive detector**: any
//! example we report an issue on is either an engine bug or a documented
//! corpus/engine quirk. That is precisely the failure mode the small inline
//! fixtures cannot catch — a fixture suite only proves we flag what we
//! decided to flag, never that we stay quiet on the ~8.7k resources the
//! ecosystem actually publishes.
//!
...
struct KnownFailure {
    ...
    issues: usize,
    kinds: Vec<String>,
    /// Optional hand-written note. Never generated; preserved by hand when
    /// a group of entries has a shared, understood root cause.
...
const MANIFEST_DESCRIPTION: &str = "Baseline for the official FHIR example corpus sweep in \
     ...
     the structural validator reports issues on -- i.e. a suspected false positive in the engine, \
     not a defect in the example. Entries may only be removed (by fixing the engine), never added \
     without review. Regenerate with the copy of this file the test writes to \
     target/spec-examples/<version>.actual.json.";
...
fn run_version(version: FhirVersion, version_dir: &str) {
    ...
    let raw = std::fs::read_to_string(&baseline_file).unwrap_or_else(|e| {
        panic!(
    ...
    let baseline: Manifest = serde_json::from_str(&raw)
