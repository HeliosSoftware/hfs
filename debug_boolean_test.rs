use sof::{SofViewDefinition, SofBundle};
use fhir::{r4::{Patient, PatientDeceased, ViewDefinition, ViewDefinitionSelect, ViewDefinitionColumn, ViewDefinitionWhere, ViewDefinitionConstant, ViewDefinitionConstantValue, Boolean}, FhirResource};
...
fn main() {
    ...
    let is_deceased_constant = fhirpath_support::EvaluationResult::Boolean(true, Some(fhirpath_support::TypeInfoResult::new("FHIR", "boolean")));
    ...
    match evaluate_expression("deceased.ofType(boolean).exists() and deceased.ofType(boolean) = %is_deceased", &context) {
