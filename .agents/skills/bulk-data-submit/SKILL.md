...
| `HFS_BULK_SUBMIT_DEFER_INDEXING` | `false` | Bulk fast-load (#903): ingest without search-index/FTS writes, then rebuild with an automatic per-type reindex when each manifest finishes. A restart before that rebuild lands leaves the data stored but unsearchable |
