...
\echo ''
\echo '######## AM. LIVE INDEX SET on search_index (must show none of the four) ########'
...
\echo ''
\echo '######## AN. REFERENCE Observation?subject=<ref> — does text_pattern_ops serve `=`? ########'
...
\echo ''
\echo '######## AO. AN COUNTERFACTUAL — the same shape with idx_search_reference back ########'
...
\echo ''
\echo '######## AP. TOKEN Observation?code:text=blood — the token_display shape ########'
...
\echo ''
\echo '######## AQ. REFERENCE Observation?subject:code-text=a — the reference_display shape ########'
...
\echo ''
\echo '######## AR. v27 INDEX ENTRIES REMOVED — rows x indexes each row stopped entering ########'
